...
void p1::BossDuck::Update(float deltaTime)
{
    ...
    glm::mat4 newMatrix = Scale(modelMatrix, glm::vec3(scene->GetSceneCamera()->GetProjectionInfo().width * 0.8f, 0.5f, 1));

    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    scene->RenderMesh(meshes["bar"], scene->GetShader("VertexColor"), newMatrix);
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

    glm::mat4 newMat2 = Scale(modelMatrix, glm::vec3(scene->GetSceneCamera()->GetProjectionInfo().width * 0.8f * health / 3, 0.5f, 1));
