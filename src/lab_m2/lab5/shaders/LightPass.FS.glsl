...
void main()
{
    ...
    // Sample the texture 'texture_position' at tex_coord to obtain
    // the world space position of the light source.
    vec3 wPos = texture(texture_position, tex_coord).xyz;

    // Sample the texture 'texture_normal' at tex_coord to obtain
    // the world space normal vector of the light source.
    vec3 wNorm = texture(texture_normal, tex_coord).xyz;

    // Compute out_color.rgb using 'PhongLight' method with the world space position 
